...
from .llama_gpt_disc import GPT_models
...
class LlamaVAR(ModelMixin, ConfigMixin):
    ...
    def forward(self, batch):
        ...
        # make the prediction
        _, loss, _ = self.transformer(idx=target_indices[:,:-1], cond_idx=text_emb, 
                                    token_embeddings=None, mask=attn_mask[:, :, :-1,:-1], targets=target_indices)
    ...
    def validate(self, batch, **sampling_kwargs):
        ...
        # differently because we are conditioning)
        # text_emb = self.caption_embeder(text_emb, train=self.training)
        # pre_vis_tokens = self.pre_transformer(vis_tokens)
        # token_embedings = torch.cat([text_emb, vis_tokens], dim=1)
        # make the prediction
        logits, _, _, = self.transformer(idx=target_indices[:, :-1], cond_idx=text_emb, 
        ...
        return idx, sample_idx, last_token_idx
    ...
    def validate_per_token(self, batch, sampling_kwargs):
        ...
        for i in range(num_tokens):
            if len(pred_token_idx) == 0:
                mask_len = text_emb.shape[1]
                cur_token_idxs = None
            else:
                mask_len = text_emb.shape[1] + len(pred_token_idx)
                cur_token_idxs = torch.cat(pred_token_idx, dim=1)
            logits, _, _, = self.transformer(idx=cur_token_idxs, cond_idx=text_emb, 
                                                token_embeddings=None,
        ...
        # posterior = DiagonalGaussianDistribution(mean, logvar)
            #sampling_kwargs['sample_logits'] = False
            token_idx, _ = sample(logits, **sampling_kwargs)
            token_idx = token_idx.view(-1, 1)
            pred_token_idx.append(token_idx)
            
        pred_token_idx = torch.cat(pred_token_idx, dim=1)
