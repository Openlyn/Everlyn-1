...
from .llama_gpt_ns_disc import GPT_models
...
class LlamaVAR(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.patch_nums = [(1,1,1), (1,2,2),(2,2,2),(2,3,3),(3,3,3),(3,4,4),(3,5,5),(3,6,6),(3,8,8),(3, 10, 10), (5,16,16), (5,32,32)]
        ...
        self.mask = torch.where(d >= dT, 1, 0).reshape(1, 1, self.L, self.L).contiguous().to(torch.bool)
        ...
        self.pos_start = nn.Parameter(torch.empty(1, self.num_token_first_scale, hidden_dim))
        ...
        self.transformer.construct_position_embeddings(self.L)
    ...
    def prepare_ground_truth(self, vae, latents):
        ...
        for si, (pt, ph, pw) in enumerate(patch_thws):
            ...
            embeddings_list.append(rearrange(embeddings, 'b c t h w -> b (t h w) c'))
    ...
    def prepare_inputs(self, vae, idx_list):
        ...
        for si in range(SN-1):
            ...
            next_input = rearrange(F.interpolate(embeddings, size=next_ps, mode='trilinear'), 'b c t h w ->b (t h w) c')
    ...
    def forward(self, batch):
        ...
        next_scale_input = self.prepare_inputs(vae, target_indices)
        
        B = text_emb.shape[0]
        # B L C, L= sum all scale tokens without text
        text_emb = self.caption_embeder(text_emb, train=self.training)
        level1_input = text_emb.expand(B, self.num_token_first_scale, -1) + self.pos_start.expand(B, self.num_token_first_scale, -1)
        ...
        # make the prediction
        assert level1_input.shape[1] + next_scale_input.shape[1] == target_indices.shape[1]
        scale_emb = self.scale_embeddings(self.lvl_pos_emb_idx.to(text_emb.device)).expand(B, -1, -1)
        level1_input = level1_input + scale_emb[:,:level1_input.shape[1]]
        next_scale_input = next_scale_input + scale_emb[:,level1_input.shape[1]:]
        attn_mask = self.mask.expand(B, -1, -1, -1).to(text_emb.device)
        
        _, loss, _ = self.transformer(idx=None, cond_idx=None, token_embeddings=next_scale_input, 
                                      cond_embeddings=level1_input, mask=attn_mask, targets=target_indices)
    ...
    def validate(self, batch, **sampling_kwargs):
        ...
        logits, _, _ = self.transformer(idx=None, cond_idx=None, token_embeddings=next_scale_input, 
                                      cond_embeddings=level1_input, mask=attn_mask)

        last_scale = self.patch_nums[-1]
        t, h, w = last_scale
        num_token_last_scale = np.prod(last_scale)
        logits = logits[:, -num_token_last_scale:]
        sample_idx, _ = sample_all(logits, **sampling_kwargs)
        sampling_kwargs['sample_logits'] = False
        idx, _ = sample_all(logits, **sampling_kwargs)
        
        target_indices = target_indices[:, -num_token_last_scale:]
        last_token_idx = torch.cat([target_indices[:,:-100], sample_idx[:,-100:]], dim=1)
        ...
        orig_idx = rearrange(target_indices, 'b (t h w) -> b t h w', t=t, h=h, w=w)
    ...
    def validate_per_token(self, batch, sampling_kwargs):
        ...
        for si in range(len(self.patch_nums)):
            ...
            if cur_scale_token_idx is not None:
                ...
                next_input = rearrange(F.interpolate(embeddings, size=ps, mode='trilinear'), 'b c t h w ->b (t h w) c')
                next_input = self.pre_transformer(next_input)
                next_input = next_input + scale_emb[:, cur_token_pos:cur_token_pos+num_token_cur_scale]
                if next_scale_input is None:
                    next_scale_input = next_input
                else:
                    next_scale_input = torch.cat([next_scale_input, next_input], dim=1)    
                mask_len = level1_input.shape[1] + next_scale_input.shape[1] 
                
            logits, _, _, = self.transformer(idx=None, cond_idx=None, 
                                            token_embeddings=next_scale_input, cond_embeddings=level1_input,
                                            mask=attn_mask[:, :, :mask_len,:mask_len])
        ...
        pred_token_idx = pred_token_idx[-1]
        t, h, w = self.patch_nums[-1]
        pred_token_idx = rearrange(pred_token_idx, 'b (t h w) -> b t h w', t=t, h=h, w=w)
