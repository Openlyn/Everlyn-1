...
from .llama_gpt_ns_disc_class_msf_visemb import GPT_models
...
class LlamaVAR(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.patch_nums = [(1,1,1), (5,1,1), (5,2,2),(5,3,3),(5,4,4),(5,5,5),(5,6,6),(5,8,8),(5, 10, 10), (5,16,16), (5,32,32)]
        ...
        self.transformer.construct_position_embeddings(int(self.L - self.num_token_first_scale))
    ...
    #         next_input = rearrange(F.interpolate(f_sum, size=next_ps, mode='trilinear'), 'b c t h w -> b ( t h w) c')
    ...
    #         idx = idx_list[si]
    #         # embeddings = vae.codebook.dictionary_lookup(idx) # B L C
    #         embeddings = self.tok_embeddings(idx)
    #         embeddings = rearrange(embeddings, 'b (t h w) c -> b c t h w', t=next_ps[0], h=next_ps[1], w=next_ps[2])
    #         embeddings = F.interpolate(embeddings, size=(t, h, w), mode='trilinear')
    #         if f_sum is None:
    #             f_sum = embeddings
    ...
    #             f_sum += embeddings
    #         next_ps = self.patch_nums[si + 1]
    #         next_input = rearrange(F.interpolate(f_sum, size=next_ps, mode='area'), 'b c t h w -> b ( t h w) c')
    #         next_scales.append(next_input)
    #     return next_scales
    def prepare_inputs(self, vae, idx_list):
        next_scales = []
        ...
        for si in range(SN-1):
            ...
            next_ps = self.patch_nums[si + 1]
            # embeddings = vae.codebook.dictionary_lookup(idx) # B L C
            embeddings = self.transformer.tok_embeddings(idx)
    ...
    def forward(self, batch):
        ...
        text_emb = self.transformer.cls_embedding(batch['label_ids'], train=True)
        ...
        text_mask = batch['text_mask'][:,-self.num_text_tokens:] # B 120 
        ...
        logits, loss, _ = self.transformer(idx=None, cond_idx=None, token_embeddings=next_scale_input, 
                                      cond_embeddings=text_emb, mask=attn_mask, targets=target_indices)
    ...
    def validate(self, batch, **sampling_kwargs):
        ...
        # else:
        #     print('minor ', vis_tokens.shape)
        text_emb = self.transformer.cls_embedding(batch['label_ids'],train=False)
        vae = batch['vae']
        # pixel_values = batch['pixel_values']
        # is_image = pixel_values.ndim == 4
        # latents, _ = vae.encode(pixel_values, is_image, include_embeddings=True)
        ...
        logits, _, _ = self.transformer(idx=None, cond_idx=None, token_embeddings=next_scale_input, cond_embeddings=text_emb, mask=attn_mask)
    ...
    def validate_per_token(self, batch, sampling_kwargs):
        ...
        for si in range(len(self.patch_nums)):
            ...
            if cur_scale_token_idx is not None:
                ...
                embeddings = self.transformer.tok_embeddings(cur_scale_token_idx)
                ...
                if f_sum is None:
                    ...
                    f_sum = F.interpolate(f_sum, size=ps, mode='trilinear') + embeddings
                next_input = rearrange(f_sum, 'b c t h w -> b (t h w) c')
            ...
            logits, _, _, = self.transformer(idx=None, cond_idx=None, 
                                            token_embeddings=next_scale_input, cond_embeddings=text_emb,
