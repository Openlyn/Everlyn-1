...
layer_idx
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # print("idx",idx)
            # # print("layer_outputs",layer_outputs.keys())
            ...
            #         last_attention_map = torch.load(os.path.join('/mnt/sda/feilongtang/Hallucination/SID/results/exp/15883', 
            #                                                         f'attn_map_{attention_map.shape[-1]-1}_layer{idx}.pt'))
            #         padding = (0, 1, 0, 1)  # Pad one element at the end of the last dimension (fourth dimension)
            #         print("last_attention_map",last_attention_map.shape)
            #         print(torch.isnan(last_attention_map).sum())
            #         new_attention_map = torch.nn.functional.pad(last_attention_map, padding)
            #         print("new_attention_map",new_attention_map.shape)
            #         print(torch.isnan(new_attention_map).sum())
            #         print(torch.isnan(attention_map).sum())
            #         # print("new_attention_map", new_attention_map.shape)
            #         # print("attention_map", attention_map.shape)
            #         new_attention_map[:,:,-1,:] = attention_map[:,:,-1,:]
            ...
            #                         f'/mnt/sda/feilongtang/Hallucination/SID/results/exp/15883/attn_map_{attention_map.shape[-1]}_layer{idx}.pt')
